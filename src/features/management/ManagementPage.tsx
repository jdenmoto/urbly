    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
            ...
            <Input label={t('management.contactPhone')} error={errors.contactPhone?.message} {...register('contactPhone')} />
      ...
      <Modal open={editOpen} title={t('management.editTitle')} onClose={() => setEditOpen(false)}>
        <form onSubmit={handleEditSubmit(onEditSubmit)} className="space-y-4">
          ...
          />
          <Input label={t('management.email')} type="email" error={editErrors.email?.message} {...editRegister('email')} />
          <Input
            label={t('management.nit')}
            placeholder={t('management.nitPlaceholder')}
            pattern="^\\d{6,12}-\\d$"
