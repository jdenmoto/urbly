...
import type { ServiceOrderChecklistValue, ServiceOrderReport } from '@/core/models/serviceOrder';
...
export async function uploadIssuePhotos(serviceOrderId: string, issueId: string, photos: File[]) {
  const uploads = await Promise.all(
    photos
      .filter((file): file is File => file instanceof File)
      .map(async (file, index) => {
        const storageRef = ref(storage, `service-orders/${serviceOrderId}/issues/${issueId}/${index}-${safeStorageName(file.name)}`);
...
export async function uploadCompletionPhotos(serviceOrderId: string, photos: File[]) {
  const uploads = await Promise.all(
    photos
      .filter((file): file is File => file instanceof File)
      .map(async (file, index) => {
        const storageRef = ref(storage, `service-orders/${serviceOrderId}/completion-photos/${Date.now()}-${index}-${safeStorageName(file.name)}`);
        await uploadBytes(storageRef, file);
        return getDownloadURL(storageRef);
      })
  );
...
export async function buildCompletionPayload(args: {
  ...
  const { serviceOrderId, hasIssues, issues, completionPhotos, completionReport, normalizedChecklist } = args;
  const completionPhotoUrls = await uploadCompletionPhotos(serviceOrderId, completionPhotos);
  ...
  if (hasIssues === 'yes') {
    const resolvedIssues = await Promise.all(
      issues.map(async (issue) => {
        const photoUrls = await uploadIssuePhotos(serviceOrderId, issue.id, issue.photos);
        return {
          id: issue.id,
          type: issue.type,
