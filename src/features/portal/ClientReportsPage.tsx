...
function downloadName(index: number) {
  return index === 0 ? 'PDF principal' : `PDF ${index + 1}`;
}
...
export default function ClientReportsPage() {
  ...
  if (!administrationId) {
    return <EmptyState title="Reportes del cliente" description={t('portal.missing.access')} />;
  }

  return (
    <div className="space-y-8">
      <PageHeader
        title="Reportes del cliente"
        subtitle={administration ? `Informes y PDFs visibles para ${administration.name}.` : 'Informes técnicos visibles para cliente.'}
        actions={
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Informes visibles" value={reportItems.length} hint="Con cierre, evidencia o PDF" />
        <StatCard label="Finalizados" value={summary.finalReports} hint="Completados o aprobados" />
        <StatCard label="PDFs adjuntos" value={summary.pdfs} hint="Documentos enlazados" />
        <StatCard label="Evidencia" value={summary.photos} hint="Fotos publicadas" />
      </section>
      ...
      <Card className="space-y-5 p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-xl font-semibold text-ink-900">Entregables técnicos</h2>
            <p className="max-w-2xl text-sm leading-6 text-ink-600">
              Lista limitada al alcance del cliente. Incluye informes generados desde servicios visibles y PDFs adjuntos publicados.
            </p>
          </div>
          <div className="rounded-2xl border border-fog-200 bg-fog-50 px-4 py-3 text-sm text-ink-600">
            ...
            <p>Novedades documentadas en reportes visibles.</p>
          </div>
        </div>
        ...
        {reportItems.length ? (
          <div className="space-y-5">
            {reportItems.map((item) => {
              ...
              const building = scopedBuildings.find((entry) => entry.id === serviceOrder.buildingId);
              return (
                <article key={serviceOrder.id} className="rounded-3xl border border-fog-200 bg-white p-5 shadow-sm">
                  <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                    <div className="min-w-0 space-y-3">
                      <div className="flex flex-wrap gap-2">
                        ...
                        <span className={`rounded-full px-3 py-1 text-xs font-semibold ${item.isFinal ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>
                          {item.isFinal ? 'Informe final' : 'Borrador visible'}
                        </span>
                      ...
                      <div>
                        ...
                        <p className="mt-2 text-sm text-ink-500">Actualizado: {formatServiceDateTime(item.lastUpdatedAt)}</p>
                      </div>
                    </div>
                    ...
                    <div className="grid gap-2 text-sm text-ink-600 sm:grid-cols-3 xl:w-[27rem]">
                      <div className="rounded-2xl bg-fog-50 p-3">
                        ...
                        <p className="mt-1 font-semibold text-ink-900">{item.photoCount} fotos</p>
                      </div>
                      <div className="rounded-2xl bg-fog-50 p-3">
                      ...
                      <div className="rounded-2xl bg-fog-50 p-3">
                        ...
                        <p className="mt-1 font-semibold text-ink-900">{item.pdfAttachments.length}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 grid gap-4 xl:grid-cols-[1.1fr,0.9fr]">
                    <div className="rounded-2xl bg-fog-50 p-4">
                      <p className="text-xs uppercase tracking-wide text-ink-500">Resumen del informe</p>
                    ...
                    <div className="rounded-2xl border border-fog-200 p-4">
                      <p className="text-sm font-semibold text-ink-900">PDFs disponibles</p>
                      {item.pdfAttachments.length ? (
                        <ul className="mt-3 space-y-2 text-sm text-ink-700">
                          {item.pdfAttachments.map((url, index) => (
                            <li key={url}>
                              <a href={url} target="_blank" rel="noreferrer" className="inline-flex rounded-full border border-slate-200 px-3 py-1.5 font-semibold text-slate-700 transition hover:bg-slate-50">
                                {downloadName(index)}
                              </a>
                            </li>
                          ))}
                        ...
                        <p className="mt-2 text-sm text-ink-500">Sin PDF adjunto. El resumen técnico queda visible en esta página.</p>
                      )}
                    </div>
                  </div>
                </article>
          ...
          <EmptyState title="Sin reportes visibles" description="Todavía no hay informes, PDFs o evidencia publicada para esta administración." />
        )}
      </Card>
    </div>
  );
