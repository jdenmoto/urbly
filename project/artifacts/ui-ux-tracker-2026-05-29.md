# Urbly — UI/UX Tracker

Fecha base: 2026-05-29
Estado: activo
Referencia: `docs/implementation/urbly-v2/ui-ux-plan-implementacion-2026-05-29.md`

## Tablero de ejecución

| Fase | Estado | Owner | Inicio | Fin | Evidencia | Verificado |
|---|---|---|---|---|---|---|
| U0 Baseline/contrato | done | designer | 2026-05-29 | 2026-05-29 | docs/implementation/urbly-v2/ui-ux-visual-contract-v1.md | no |
| U1 i18n/copy hardening | done | designer+developer | 2026-05-29 | 2026-05-29 | docs/implementation/urbly-v2/u1-t01-hardcoded-copy-audit-2026-05-29.md | no |
| U2 shell/nav | done | designer+developer | 2026-05-29 | 2026-05-29 | docs/implementation/urbly-v2/u2-t05-iconography-policy-2026-05-29.md | no |
| U3 técnico móvil | done | designer+developer | 2026-05-29 | 2026-05-29 | project/runs/2026-05-29-u3-t05-tap-targets.md | no |
| U4 portal cliente | in_progress | designer+developer | 2026-05-29 | - | project/runs/2026-05-29-u4-t01-client-copy.md | no |
| U5 ai workspace | pending | designer+developer | - | - | - | no |
| U6 accesibilidad | pending | designer+developer | - | - | - | no |
| U7 cierre/verificación | pending | verify | - | - | - | no |

## Lista atómica (checklist)

- [x] U0-T01 contrato superficies
- [x] U0-T02 tokens spacing/radius/shadow/motion
- [x] U0-T03 patrón header/metrics/contexto
- [x] U0-T04 estados canónicos
- [x] U0-T05 checklist QA visual
- [x] U1-T01 auditoría strings hardcodeados
- [x] U1-T02 dashboard i18n
- [x] U1-T03 services i18n
- [x] U1-T04 portal i18n
- [x] U1-T05 technician/ai i18n
- [x] U1-T06 tests i18n
- [x] U2-T01 búsqueda global real o explicit disabled
- [x] U2-T02 notificaciones robustas
- [x] U2-T03 labels/descr por rol
- [x] U2-T04 topbar responsive
- [x] U2-T05 iconografía
- [x] U3-T01 home técnico 5s scan
- [x] U3-T02 sticky CTA móvil
- [x] U3-T03 cards cola técnica
- [x] U3-T04 feedback acciones
- [x] U3-T05 QA tap targets
- [x] U4-T01 copy cliente
- [ ] U4-T02 prioridades resumen cliente
- [ ] U4-T03 trazabilidad reciente
- [ ] U4-T04 cobertura por edificio
- [ ] U4-T05 secure portal narrative
- [ ] U5-T01 semántica visual IA
- [ ] U5-T02 disclaimers IA
- [ ] U5-T03 cards de sugerencias
- [ ] U5-T04 alineación con services
- [ ] U6-T01 focus-visible
- [ ] U6-T02 semántica botones/listas
- [ ] U6-T03 contraste etiquetas
- [ ] U6-T04 keyboard en overlays
- [ ] U6-T05 QA teclado rutas críticas
- [ ] U7-T01 validación técnica final
- [ ] U7-T02 changelog UI/UX
- [ ] U7-T03 evidencia before/after
- [ ] U7-T04 handoff a verify

## Riesgos / bloqueos

- Pendiente priorización de sprint para ejecución de fases.
- Pendiente definir owner de verificación por fase.

## Regla de cierre

Ninguna fase se marca `done` sin:
1. evidencia enlazada,
2. validación técnica,
3. confirmación de verificación.
